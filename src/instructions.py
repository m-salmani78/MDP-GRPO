...
from instruction_constants import get_instruction_constants
...
# Default language for instruction constants (can be overridden per instruction)
_DEFAULT_LANGUAGE = "en"

# Get language-specific constants (will be used as defaults)
_lang_constants = get_instruction_constants(_DEFAULT_LANGUAGE)

# The relational operation for comparison.
_COMPARISON_RELATION = _lang_constants.COMPARISON_RELATION
...
_CONSTRAINED_RESPONSE_OPTIONS = _lang_constants.CONSTRAINED_RESPONSE_OPTIONS

# The options of starter keywords.
_STARTER_OPTIONS = _lang_constants.STARTER_OPTIONS

# The options of ending keywords.
_ENDING_OPTIONS = _lang_constants.ENDING_OPTIONS

# The number of highlighted sections.
_NUM_HIGHLIGHTED_SECTIONS = 4

# The section spliter.
_SECTION_SPLITER = _lang_constants.SECTION_SPLITER
...
_POSTSCRIPT_MARKER = _lang_constants.POSTSCRIPT_MARKER
...
def set_instruction_language(language: str = "en"):
    """Set the language for instruction constants globally.
    
    This function allows switching between English and Persian (or other supported
    languages) for all instruction constants. It should be called before creating
    any instruction instances if you want to use a non-default language.
    ...
    Example:
        >>> set_instruction_language("fa")  # Switch to Persian
        >>> set_instruction_language("en")  # Switch back to English
    """
    global _DEFAULT_LANGUAGE, _lang_constants
    global _COMPARISON_RELATION, _CONSTRAINED_RESPONSE_OPTIONS
    global _STARTER_OPTIONS, _ENDING_OPTIONS
    global _SECTION_SPLITER, _POSTSCRIPT_MARKER
    
    _DEFAULT_LANGUAGE = language
    _lang_constants = get_instruction_constants(language)
...
def compare_with_relation(actual: int, threshold: int, relation: str) -> bool:
  """Generic comparator for all 5 relations.

  relation must be one of _COMPARISON_RELATION in this order:
  [less than, at least, more than, up to, exactly]
  """
  if relation == _COMPARISON_RELATION[0]:
    return actual < threshold
  elif relation == _COMPARISON_RELATION[1]:
    return actual >= threshold
  elif relation == _COMPARISON_RELATION[2]:
    return actual > threshold
  elif relation == _COMPARISON_RELATION[3]:
    return actual <= threshold
  elif relation == _COMPARISON_RELATION[4]:
    return actual == threshold
  return False
...
class ResponseLanguageChecker(Instruction):
  ...
  def build_description(self, *, language = None):
    ...
    self._description_pattern = (
        ...
        "language is allowed.")
    # Fall back to the raw code if not in the mapping
  ...
  def check_following(self, value):
    ...
    # If too few alphabetic characters, skip detection to avoid errors
    letters = sum(1 for ch in value if ch.isalpha())
    if letters < 3:
      return True

    try:
      # Use lang code detection and compare to requested language code
      return langdetect.detect(value) == self._language
    except langdetect.LangDetectException as e:
      # Count as instruction is followed, but avoid noisy error logs
      logging.warning(
          "Unable to detect language for given text: %s", e
      )
      return True
...
class NumberOfSentences(Instruction):
  ...
  def check_following(self, value):
    ...
    Raise:
        ...
        [`less_than`, `at_least`, `more_than`, `up_to`, `exactly`].
    """
    num_sentences = instructions_util.count_sentences(value)
    return compare_with_relation(num_sentences, self._num_sentences_threshold, self._comparison_relation)
...
class PostscriptChecker(Instruction):
  ...
  def check_following(self, value):
    ...
    elif self._postscript_marker == "پ.ن.":
      postscript_pattern = r"\s*پ\.\s?ن\..*$"
    else:
...
class KeywordFrequencyChecker(Instruction):
  ...
  def check_following(self, value):
    ...
    return compare_with_relation(actual_occurrences, self._frequency, self._comparison_relation)
...
class NumberOfWords(Instruction):
  ...
  def check_following(self, value):
    ...
    return compare_with_relation(num_words, self._num_words, self._comparison_relation)
...
class LetterFrequencyChecker(Instruction):
  ...
  def check_following(self, value):
    ...
    return compare_with_relation(letters[self._letter], self._frequency, self._comparison_relation)
...
class CommaChecker(Instruction):
  ...
  def check_following(self, value):
    ...
    return not re.search(r"[,،]", value) # Added Persian comma
...
class CapitalWordFrequencyChecker(Instruction):
  ...
  def check_following(self, value):
    ...
    words = re.findall(r"\b\w[\w\-]*\b", value)
    capital_words = [word for word in words if word.isupper()]

    capital_words = len(capital_words)

    return compare_with_relation(capital_words, self._frequency, self._comparison_relation)
...
class XmlFormat(Instruction):
  """Check that the entire output is valid XML (optionally inside ``` fences)."""

  def build_description(self):
    self._description_pattern = (
        "Entire output should be wrapped in XML format. "
        "You may optionally wrap the XML in markdown ticks (```xml ... ```)."
    )
    return self._description_pattern
  ...
  def _strip_code_fences(self, s: str) -> str:
    s = s.strip()
    # Remove leading fenced header if present: ```xml / ```XML / ``` (plus optional spaces and a newline)
    m = re.match(r"^\s*```(?:[xX][mM][lL])?\s*\n", s)
    if m:
      s = s[m.end():]
      # Remove trailing closing fence
      s = re.sub(r"\n?\s*```\s*$", "", s)
    # Also handle the simple case where user used single-line ``` wrappers without newline
    else:
      s = s.removeprefix("```xml").removeprefix("```Xml").removeprefix("```XML").removeprefix("```")
      s = s.removesuffix("```")
    return s.strip()

  def _looks_like_balanced_xml(self, xml_string: str) -> bool:
    # Basic structure: optional XML declaration, then a single root <tag> ... </tag>
    # We capture the root tag name and require the same name to close.
    pattern = r'^\s*(?:<\?xml\s[^?]*\?>\s*)?<([A-Za-z_][\w\-.:]*)\b[\s\S]*</\1>\s*$'
    return re.match(pattern, xml_string) is not None

  def check_following(self, value: str) -> bool:
    payload = self._strip_code_fences(value)

    # First, try strict XML parsing (best signal, allows CDATA, namespaces, self-closing tags, etc.)
    try:
      ET.fromstring(payload)
      return True
    except ET.ParseError:
      # Fall back to a conservative structural check to avoid false negatives due to minor issues
      return self._looks_like_balanced_xml(payload)
